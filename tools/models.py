...
class BertModelWDualMultiClassClassifierAndSegmentationHead(
...
class RobertaModelWDualMultiClassClassifierHeadV9(nn.Module):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__()
        if pretrained_model_name_or_path:
            if isinstance(pretrained_model_name_or_path, str):
...
class RobertaModelWDualMultiClassClassifierHeadV10(nn.Module):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__()
        if pretrained_model_name_or_path:
            if isinstance(pretrained_model_name_or_path, str):
        ...
        self.num_labels = num_labels
        self.dropout = nn.Dropout(0.2)
        self.classifier_conv_head_tail = nn.Conv1d(
    ...
    def forward(self, input_ids=None, attention_mask=None,
        ...
        output = torch.transpose(output, 1, 2)
        output = self.dropout(output)
        logits_head = self.classifier_conv_head_tail(output).squeeze()
        logits_tail = self.classifier_conv_head_tail(
            output.flip(
                dims=(
                    2,
                ))).squeeze().flip(
...
class RobertaModelWDualMultiClassClassifierHeadV12(nn.Module):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__()
        if pretrained_model_name_or_path:
            if isinstance(pretrained_model_name_or_path, str):
        ...
        self.classifier_conv_head = nn.Conv1d(
            self.model.pooler.dense.out_features + 1, 1, 1, padding=0)
        self.classifier_conv_tail = nn.Conv1d(
            self.model.pooler.dense.out_features + 1, 1, 1, padding=0)
...
class RobertaModelWDualMultiClassClassifierHeadV15(nn.Module):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__()
        if pretrained_model_name_or_path:
            if isinstance(pretrained_model_name_or_path, str):
...
class RobertaModelWDualMultiClassClassifierAndSegmentationHeadV2(RobertaModelWDualMultiClassClassifierHeadV2):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__(num_labels, pretrained_model_name_or_path)
        self.classifier_conv_segmentation = nn.Conv1d(
            self.model.pooler.dense.out_features, 1, 1)
    ...
    def forward(self, input_ids=None, attention_mask=None,
        ...
        # add hidden states and attention if they are here
        outputs = ((logits_head, logits_tail, logits_segmentation),)
...
class RobertaModelWDualMultiClassClassifierAndSegmentationHeadV3(RobertaModelWDualMultiClassClassifierHeadV3):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        ...
        self.dropout = nn.Dropout(0.2)
        self.classifier_head_tail = nn.Linear(
            self.model.pooler.dense.out_features * 2, 3)
    ...
    def forward(self, input_ids=None, attention_mask=None,
        ...
        output = self.dropout(output)
        logits = self.classifier_head_tail(output)
        logits_head, logits_tail, logits_segmentation = logits.split(1, dim=-1)
...
class RobertaModelWDualMultiClassClassifierAndSegmentationHeadV8(
    ...
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__(num_labels, pretrained_model_name_or_path)
        self.classifier_conv_segmentation = nn.Conv1d(
            self.model.pooler.dense.out_features, 1, 1, padding=0)
        self.add_module(
            'conv_output_segmentation',
            self.classifier_conv_segmentation)
...
class RobertaModelWDualMultiClassClassifierAndSegmentationHeadV9(
        RobertaModelWDualMultiClassClassifierHeadV9):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__(num_labels, pretrained_model_name_or_path)
        self.classifier_conv_segmentation = nn.Conv1d(
            self.model.pooler.dense.out_features, 1, 1, padding=0)
    ...
    def forward(self, input_ids=None, attention_mask=None,
        ...
        output_segmentation = outputs[2][-3]
        output_head = outputs[2][-2]
        output_tail = outputs[2][-1]

        output_segmentation = torch.transpose(output_segmentation, 1, 2)
        output_head = torch.transpose(output_head, 1, 2)
        output_tail = torch.transpose(output_tail, 1, 2)
        output_segmentation = self.dropout(output_segmentation)
        output_head = self.dropout(output_head)
        output_tail = self.dropout(output_tail)
        logits_segmentation = self.classifier_conv_head(
            output_segmentation).squeeze()
        logits_head = self.classifier_conv_head(output_head).squeeze()
        logits_tail = self.classifier_conv_tail(output_tail).squeeze()
...
class RobertaModelWDualMultiClassClassifierAndSegmentationHeadV10(
        RobertaModelWDualMultiClassClassifierHeadV10):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__(num_labels, pretrained_model_name_or_path)
        self.classifier_conv_segmentation = nn.Conv1d(
            self.model.pooler.dense.out_features, 1, 1, padding=0)
...
class RobertaModelWDualMultiClassClassifierAndSegmentationHeadV11(nn.Module):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__()
        if pretrained_model_name_or_path:
            if isinstance(pretrained_model_name_or_path, str):
        ...
        self.num_labels = num_labels
        self.dropout = nn.Dropout(0.2)
        self.classifier_conv = nn.Conv1d(
            self.model.pooler.dense.out_features, 3, 1)
    ...
    def forward(self, input_ids=None, attention_mask=None,
        ...
        output = torch.transpose(output, 1, 2)
        output = self.dropout(output)
        logits = self.classifier_conv(output)
        logits_head, logits_tail, logits_segmentation = logits.split(1, dim=1)
        logits_head = logits_head.squeeze()
        logits_tail = logits_tail.squeeze()
        logits_segmentation = logits_segmentation.squeeze()
...
class RobertaModelWDualMultiClassClassifierAndSegmentationHeadV12(
        RobertaModelWDualMultiClassClassifierHeadV12):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__(num_labels, pretrained_model_name_or_path)
        self.classifier_conv_segmentation = nn.Conv1d(
            self.model.pooler.dense.out_features, 1, 1, padding=0)
    ...
    def forward(self, input_ids=None, attention_mask=None,
        ...
        new_output = torch.cat(
            [output, self.sigmoid(logits_segmentation)], dim=1)
        logits_segmentation = logits_segmentation.squeeze()
        logits_head = self.classifier_conv_head(new_output).squeeze()
        logits_tail = self.classifier_conv_tail(new_output).squeeze()
...
class RobertaModelWDualMultiClassClassifierAndSegmentationHeadV13(nn.Module):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__()
        if pretrained_model_name_or_path:
            if isinstance(pretrained_model_name_or_path, str):
        ...
        self.num_labels = num_labels
        self.dropout = nn.Dropout(0.2)
        # self.classifier_head = nn.Parameter(torch.FloatTensor(
        #     self.model.pooler.dense.out_features * 2, 1))
        # self.classifier_tail = nn.Parameter(torch.FloatTensor(
        #     self.model.pooler.dense.out_features * 2, 1))
        # self.classifier_segmentation = nn.Parameter(torch.FloatTensor(
        #     self.model.pooler.dense.out_features * 2, 1))
        self.classifier_head = nn.Parameter(torch.FloatTensor(
            self.model.pooler.dense.out_features, 1))
        self.classifier_tail = nn.Parameter(torch.FloatTensor(
            self.model.pooler.dense.out_features, 1))
        self.classifier_segmentation = nn.Parameter(torch.FloatTensor(
            self.model.pooler.dense.out_features, 1))
        nn.init.xavier_uniform_(self.classifier_head)
        nn.init.xavier_uniform_(self.classifier_tail)
        nn.init.xavier_uniform_(self.classifier_segmentation)
    ...
    def forward(self, input_ids=None, attention_mask=None,
        ...
        output1 = F.normalize(outputs[2][-3])
        output2 = F.normalize(outputs[2][-2])
        output3 = F.normalize(outputs[2][-1])
        # output = self.dropout(output)
        # logits_head = F.linear(output, F.normalize(self.classifier_head))
        # logits_tail = F.linear(output, F.normalize(self.classifier_tail))
        # logits_segmentation = F.linear(output, F.normalize(self.classifier_segmentation))
        logits_segmentation = (output1 @ F.normalize(self.classifier_segmentation)).squeeze()
        logits_head = (output2 @ F.normalize(self.classifier_head)).squeeze()
        logits_tail = (output3 @ F.normalize(self.classifier_tail)).squeeze()
...
class RobertaModelWDualMultiClassClassifierAndSegmentationHeadV14(
        RobertaModelWDualMultiClassClassifierHead):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__(num_labels, pretrained_model_name_or_path)
        self.classifier_conv_segmentation = nn.Conv1d(
            self.model.pooler.dense.out_features, 1, 1)
        self.classifier_conv_single = nn.Conv1d(
            self.model.pooler.dense.out_features, 1, 1)
    ...
    def forward(self, input_ids=None, attention_mask=None,
        ...
        logits_segmentation = self.classifier_conv_segmentation(
            output).squeeze()
        logits_single = self.classifier_conv_single(
            output).squeeze()
        ...
        # add hidden states and attention if they are here
        outputs = ((logits_head, logits_tail,
                    logits_segmentation, logits_single),)

        return outputs  # logits, (hidden_states), (attentions)


class RobertaModelWDualMultiClassClassifierAndSegmentationHeadV15(
        RobertaModelWDualMultiClassClassifierHeadV15):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__(num_labels, pretrained_model_name_or_path)
        self.classifier_conv_segmentation = nn.Conv1d(
            self.model.pooler.dense.out_features, 1, 1)
    ...
    def forward(self, input_ids=None, attention_mask=None,
        ...
        output = self.dropout(output)
        logits_head = self.classifier_conv_head(output).squeeze()
        logits_tail = self.classifier_conv_tail(output.flip(dims=(1, ))).flip(dims=(1, )).squeeze()
        logits_segmentation = self.classifier_conv_segmentation(
            output).squeeze()
...
class RobertaModelWDualMultiClassClassifierAndCumsumSegmentationHeadV2(
        RobertaModelWDualMultiClassClassifierHead):
    def __init__(self, num_labels, pretrained_model_name_or_path):
        super().__init__(num_labels, pretrained_model_name_or_path)
        self.softmax = nn.Softmax(dim=1)
